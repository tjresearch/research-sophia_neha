...
def frame_parse(frame_num):
    #json_frame = open \
    #    ("../output/video_output/VID_TEST_CASE_1_keypoints/VID_TEST_CASE_1_{0:012d}_keypoints.json".format(frame_num), 'r')
    json_frame = open(frame_num)
    ...
    for keys in people:	# keyval: people -> hand,face keypoints etc. part_candidates->candidates for
        ...
        for key in keys:
            if(key == "person_id"):
                continue
            print(key)
            if key == 'pose_keypoints_2d' and len(frame_angles) ==0: # to do only the first pose keypoints set.. idk which one to do
                keypoints = keys[key]
                ...
                for angval in body_angle_key: #to calculate all angles in frame
                    ...
                    ang = angle_calc(grouped[v[0]][0], grouped[v[0]][1], grouped[v[1]][0], grouped[v[1]][1], grouped[v[2]][0], grouped[v[2]][0])
